...
export function StackProtectionBadge({type}) {
  ...
  const progress = useMemo(() => {
    if (type === 'miss') {
      if (state === IdentityStatus.Candidate) {
        return {color: red, value: 0.1}
      }

      if (state === IdentityStatus.Newbie) {
        if (age === 1) {
          return {color: red, value: 0.3}
        }
        if (age === 2) {
          return {color: red, value: 0.6}
        }
        if (age >= 3) {
          return {color: red, value: 0.9}
        }
      }
    ...
    if (type === 'fail') {
      ...
      if (state === IdentityStatus.Verified) {
        return {color: red, value: 0.94}
      }

      if (state === IdentityStatus.Human) {
        return {color: blue, value: 1}
      }

      if (state === IdentityStatus.Suspended && age === 4) {
        return {color: red, value: 0.1}
      }
      ...
      if (
        ...
        return {color: red, value: 0.9 + age / 100}
      }
