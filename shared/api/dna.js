...
export async function fetchFlip(hash) {
  console.debug(`flip_get request`, hash)
  const {data} = await api().post('/', {
    method: 'flip_get',
    params: [hash],
    id: 1,
  })
  console.debug(`flip_get response`, hash)
  return data
}
...
export async function fetchRawFlip(hash) {
  ...
  console.debug(`flip_getRaw response`, hash)
  return data
}
...
export async function fetchFlipKeys(addr, hash) {
  ...
  console.debug(`flip_getKeys response`, hash)
  return data
}
